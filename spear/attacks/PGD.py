...
class PGD():
    """
    The Project Gradient Descent attack for generating adversarial examples as described in 'Towards Deep Learning
    Models Resistant to Adversarial Attacks' by Madry et al.
    """
    ...
    def generate(self, net, images, labels = None):
        """
        Generates adversarial examples using Projected Gradient Descent algorithm.
        ...
        with torch.no_grad():
            ...
            images = images + 2 * self.eps * (torch.rand(images.size(), device=images.device) - 0.5)
